...
def get_weather(city: str) -> str:
    ...
    Examples:
        get_weather("北京") 返回 "北京: 多云, 15-22°C, 空气质量:良, 湿度:45%"
    """
    weather_db = {
        "北京": "多云, 15-22°C, 空气质量良, 湿度 45%",
        "上海": "晴天, 18-25°C, 空气质量优, 湿度 60%",
        "深圳": "小雨, 22-28°C, 空气质量优, 湿度 75%",
        "成都": "阴天, 16-23°C, 空气质量良, 湿度 70%",
        "杭州": "晴天, 17-24°C, 空气质量优, 湿度 55%",
        "广州": "多云, 21-29°C, 空气质量良, 湿度 72%"
    }
...
class SmartAssistant:
    """多功能智能助手"""
    def __init__(self, model, tool_list):
        # 初始化模型
        self.model = model
    ...
    def chat(self, user_input: str):
        ...
        # 组合流式模式：messages 逐 token，values 拿完整状态
        for mode, data in self.agent.stream(
            ...
            if mode == "messages":
                ...
                if isinstance(message_chunk, AIMessageChunk) and message_chunk.content:
                    # 过滤 tool 调用产生的中间文本（可选）
                    # 如果你想看到所有过程，去掉下面这个 if 判断即可
                    if not getattr(message_chunk, "tool_call_chunks", None):
                        print(message_chunk.content, end="", flush=True)
                        full_response += message_chunk.content

            elif mode == "values":
                # 每次状态更新，保存最新的 messages
